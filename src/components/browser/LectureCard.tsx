      ...
      <button
        ...
        className={`text-left rounded-2xl p-5 shadow-sm border transition hover:shadow-md hover:-translate-y-0.5 w-full ${
          highlight
            ...
            : 'border-coral-100 bg-white'
        }`}
      >
        {rank && (
          <div className="flex items-center gap-2 mb-2">
            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-coral-400 text-white text-xs font-bold">
              {rank}
            </span>
            <span className="text-xs text-coral-500 font-medium">おすすめ</span>
          </div>
        )}
        <div className="mb-2">
        ...
        <div className="flex flex-wrap gap-1 mb-2">
          {lecture.ageGroups.map((a) => (
            <span key={a} className="text-xs px-2 py-0.5 rounded-full bg-sky-100 text-sky-700">
              {AGE_LABELS[a]}
            </span>
          ))}
        </div>
